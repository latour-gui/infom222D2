...
#'
#' This will simulate a queue of `n` jobs (which have their interarrivals time generated by a normal law)
#' beeing processed by a server (which service time is generated by another normal law) and that have a
#' certain probability of beeing replaced in the queue after service completion.
#'
#' Once the arrivals queue is generated, the idea is to consider a time `t` from the queue and process
#' its delay and service time by jumping in time.
#' We keep track of the server's usage via the `busy` value which contain the time until when the server
#' is busy.
#'
...
#'                               - 1 meaning that every jobs are put in the queue after service
#'                                  completion
...
#'                          - n_arrivals (integer) : the number of initial arrivals (without the
#'                              loopback)
...
#'                          - average_delay (float): the average time that a job spend in the queue of
#'                              the system
#'                            note that the `average_delay` depend on the number of initial arrivals
#'                          - average_service_time (float) : the average time that a job spend in the
#'                              server
#'                            also note that the `average_service_time` depend only on the number of
#'                              initial arzrivals
ssqlb <- function(n, loopback_p = 0.1, arrival_mean = 30,
                  arrival_sd = 10, service_mean = 30, service_sd = 10) {
